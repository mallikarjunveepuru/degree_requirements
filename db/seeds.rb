...
def course_edges(parent, courses)
  courses.each do |course|
    Edge.create(parent_id: parent.id, child_id: Course.find_by_name(course).id, child_klass: "Course")
  end
end
...
add_link = proc { |big_node, nodes| nodes.each { |node| Edge.create(parent_id: big_node.id, child_id: node.id, child_klass: "Requirement") } }
...
edges.each { |big_node, node| Edge.create(parent_id: Requirement.find(&by_name.curry.(big_node)).id, child_id: Requirement.find(&by_name.curry.(node)).id, child_klass: "Requirement") }
